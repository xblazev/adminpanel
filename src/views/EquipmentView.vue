...
const form = ref({})
const dialogTitle = computed(() => (editingId.value ? 'Edit Equipment' : 'Tambah Equipment'))
...
const saveEquipment = () => {
  ...
  window.setTimeout(() => {
    if (editingId.value) {
      rows.value = rows.value.map((row) => (row._id === editingId.value ? { ...form.value } : row))
...
<template>
  <AdminShell>
    <v-container fluid class="pa-6">
      <div class="d-flex flex-wrap justify-space-between align-center ga-4 mb-6">
        <div>
          ...
          <div class="text-soft mt-1">Kelola equipment dummy berdasarkan struktur response.</div>
        </div>
        <div class="d-flex ga-3">
          ...
          <v-btn color="primary" prepend-icon="mdi-plus" @click="openCreate">Tambah</v-btn>
        </div>
      </div>
    ...
    <v-dialog v-model="editDialog" max-width="980" transition="slide-y-transition">
      <v-card rounded="xl">
        ...
        <v-card-actions class="px-6 pb-6">
          ...
          <v-btn color="primary" :loading="saving" @click="saveEquipment">Simpan</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="deleteDialog" max-width="420">
      <v-card rounded="xl">
        ...
        <v-card-text class="px-6">
          Data <strong>{{ deletingRow?.equipment_id }}</strong> akan dihapus dari tabel.
        </v-card-text>
        <v-card-actions class="px-6 pb-6">
          <v-spacer />
          <v-btn variant="text" @click="deleteDialog = false">Batal</v-btn>
          <v-btn color="error" :loading="deleting" @click="removeEquipment">Hapus</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>

    <v-snackbar v-model="snackbar" color="secondary" timeout="2200" location="top">
      {{ snackbarMessage }}
    </v-snackbar>
  </AdminShell>
</template>
